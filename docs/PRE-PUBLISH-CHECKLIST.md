...
- Verify the following are present:
  ...
  - `lib/canonical.js`
  - `business_modules/mcp-server/` (entry point and dependencies)
...
- Confirm the MCP server runs when Cursor invokes it. Path should be `<extensionPath>/business_modules/mcp-server/index.js`, where `<extensionPath>` is the installed extension directory (e.g. under Cursor’s `extensions` folder).
- The `@modelcontextprotocol/sdk` dependency must resolve from the extension’s `node_modules` when Cursor runs `node .../business_modules/mcp-server/index.js`.
